...
return new class extends Migration {
    ...
    public function up()
    {
        Schema::create('patients', function (Blueprint $table) {
            ...
            $table->date('dob')->nullable()->comment('number only');
            $table->integer('age')->nullable()->comment('number only');

            $table->integer('gender')->nullable()->comment('M = Male,F = Female');
            $table->string('marital_status')->nullable()->fulltext()->comment('S = Single,D = Divorce, M = Married');
            $table->string('blood_group')->nullable();
            $table->string('email')->nullable()->comment('because one email can be use for multiple patient from the same house');
            $table->string('phone')->nullable()->comment('because one phone number can be use for multiple patient');
            ...
            $table->string('same_a_patient')->default(0)->comment('if the same as patient all the address will copy from the patient to the kin of the box ');
